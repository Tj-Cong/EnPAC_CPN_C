...
/*********************************************************/
/*对于变迁cpnet->transition[fi.cur_tranid]，寻找他的下一个绑定，
 * 如果没有绑定了则返回false；
 * */
bool CPN_Product::getNextBinding() {
    const CTransition &tt = cpnet->transition[this->fi.cur_tranid];
    for(int i=tt.relvararray.size()-1;i>=0;--i)
    {
        //i表示变迁tt的第i个变量
        const Variable &var = tt.relvararray[i];
        fi.cur_varvec[var.vid] = fi.cur_varvec[var.vid]+1;
        if(fi.cur_varvec[var.vid]>=var.upbound)
        {
            fi.cur_varvec[var.vid] = 0;
            continue;
        } else{
            return true;
        }
...
/*寻找下一个可能会发生的变迁（粗过滤）
 * */
bool CPN_Product::getNextTrans() {
    do {
        fi.cur_tranid++;
        if(fi.cur_tranid>=transitioncount)
            break;
        //预检测
        CTransition &tran = cpnet->transition[fi.cur_tranid];
        vector<CSArc>::iterator piter;

        bool possiblefire = true;
        for(piter=tran.producer.begin();piter!=tran.producer.end(); ++piter)
        {
            if(RGname_ptr->marking[piter->idx].colorcount == 0)
            {
                possiblefire = false;
                break;
            }
    ...
    } while(fi.cur_tranid<transitioncount);
    return false;
}

/*function:生成当前状态的一个后记状态；
 * Algorithm:
 * 1.如果virgin=true(表示当前变迁一个绑定还未生成过)；
 * 则判断初始绑定是否可以发生；
 * 2.virgin=false;
 *      i.当前变迁没有
 * 调用getNextBind或getNextTrans得到下一个可发生的变迁或绑定（如果返回false，则表示没有后继状态了）
 * 3.生成当前fire_info下的后继状态
 * */
CPN_RGNode *CPN_Product::getNextRGNode(bool &exist) {
    while(fi.cur_tranid<transitioncount)
    {
        if(ready2exit)
            return NULL;
        const CTransition &tran = cpnet->transition[fi.cur_tranid];
        //先看看初始绑定能否发生
        if(fi.virgin)
        {
            fi.virgin = false;
            if(this->RGname_ptr->isFiringBinding(fi.cur_varvec,tran))
            {
                //生成后继状态
                dink = false;
                return getChild(fi.cur_varvec,tran,exist);
            }
        }
...
CPN_RGNode *CPN_Product::getChild(const COLORID *varvec, const CTransition &tran,bool &exist) {
    ...
    *newmark = *this->RGname_ptr;
    ...
    for(i=0,front=tran.producer.begin();front!=tran.producer.end();++i,++front)
    {
        ...
        computeArcEXP(front->arc_exp,ms_pre,varvec,psnum);
    ...
    for(i=0,rear=tran.consumer.begin();rear!=tran.consumer.end();++i,++rear)
    {
        ...
        computeArcEXP(rear->arc_exp,ms_post,varvec,psnum);
...
void CPN_Product_Automata::TCHECK(CPN_Product *p0) {
    ...
    while(!dstack.empty() && !ready2exit)
    {
        ...
        CPN_Product *p = cstack.mydata[ddtop];
...
unsigned short CPN_Product_Automata::ModelChecker(string propertyid,unsigned short each_run_time) {
    ...
    if(timeflag && memory_flag && stack_flag)
    {
        if(result)
        {
            ...
            cout << "FORMULA " << propertyid << " " << re + " TECHNIQUES SEQUENTIAL_PROCESSING ABSTRACTIONS EXPLICIT";
            ret = 1;
        }
        else
        {
