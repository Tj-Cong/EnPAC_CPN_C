...
struct fire_info
{
    bool virgin;        //virgin=true(表示当前变迁一个绑定还未生成过)；
    SHORTNUM cur_tranid;
    COLORID cur_varvec[MAXVARNUM];
};
...
public:
    CPN_Product() {
        if(varcount>MAXVARNUM) {
            cerr<<"The number of variables is over "<<MAXVARNUM<<endl;
            exit(-1);
        }
        id = 0;
        RGname_ptr = NULL;
    ...
    CPN_RGNode *getChild(const COLORID *varvec,const CTransition &tran,bool &exist);
